          ...
          <Accordion type="single" collapsible className="space-y-4">
            {modulos.map((modulo, index) => (
              <AccordionItem 
                key={index} 
                value={`modulo-${index}`}
                className="bg-card rounded-xl border border-border shadow-soft px-6"
              >
                <AccordionTrigger className="text-left font-semibold text-foreground hover:no-underline py-6">
                  <span className="text-base md:text-lg">{modulo.titulo}</span>
                </AccordionTrigger>
                <AccordionContent className="pb-6">
                  <ul className="space-y-3">
                    {modulo.topicos.map((topico, topicoIndex) => (
                      <li key={topicoIndex} className="flex items-start gap-3 text-muted-foreground">
